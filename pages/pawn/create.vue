  ...
  <div class="max-w-7xl mx-auto px-10">
    <div v-if="examination">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-6 border border-gold">
        ...
        <tbody>
          <tr v-for="gold of examination.golds" :key="gold.id"
            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 border-gold">
            <td scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">
              {{ gold.id }}
            </td>
      ...
      <form class="mt-8 border border-gold rounded px-6 py-5">
        ...
        <div class="relative z-0 w-full mb-6 group">
          <input type="number" name="loanAmount" id="loanAmount" v-model="loanAmount"
          ...
          <div class="text-red-500 text-sm">{{ errorMessage.loanAmount }}</div>
        </div>
        <div class="relative z-0 w-full mb-6 group">
        ...
        <div class="relative z-0 w-full mb-6 group">
          <label for="shopPayoutType" class="text-sm">เลือกรูปแบบการชำระเงิน</label>
          <select id="shopPayoutType" v-model="shopPayoutType"
            class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
            <option disabled value="">รูปแบบการชำระเงิน</option>
            <option value="cash">เงินสด</option>
            <option value="transaction">เงินโอน</option>
          </select>
          <div class="text-red-500 text-sm">{{ errorMessage.shopPayoutType }}</div>
        </div>
        <div class="relative z-0 w-full mb-6 group">
          <input type="text" name="customerAccount" id="customerAccount" v-model="customerAccount"
            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            placeholder=" " required />
          ...
          <div class="text-red-500 text-sm">{{ errorMessage.customerAccount }}</div>
        </div>
